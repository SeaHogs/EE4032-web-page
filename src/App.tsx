...
function App() {
  ...
  const initialMarketplaceAddress =
    (import.meta.env.VITE_MARKETPLACE_ADDRESS as string | undefined)?.trim() ?? ''

  const [contractAddress, setContractAddress] = useState<string>(initialMarketplaceAddress)
  const [marketplaceStatus, setMarketplaceStatus] = useState<string>(() =>
    initialMarketplaceAddress && isAddress(initialMarketplaceAddress)
      ? 'Ready to interact with the marketplace contract.'
      : 'Enter a marketplace contract address to begin.',
  )
  ...
  const isValidContractAddress = useMemo(() => isAddress(contractAddress), [contractAddress])
  const typedAddress = useMemo(
    () => (isValidContractAddress ? (contractAddress as Address) : undefined),
  ...
  useEffect(() => {
    if (!contractAddress) {
      setMarketplaceStatus('Enter a marketplace contract address to begin.')
      setFetchedTask(null)
    ...
    if (!isValidContractAddress) {
      setMarketplaceStatus('The provided contract address is invalid.')
    }
  }, [contractAddress, isValidContractAddress])
  ...
  const requireReadyMarketplace = useCallback((): Address | null => {
    if (!typedAddress) {
      setMarketplaceStatus('Enter a valid marketplace contract address to continue.')
      return null
    }
    ...
    async (event: FormEvent<HTMLFormElement>) => {
      ...
      try {
        ...
        if (minJurorStake && normalizedStake < minJurorStake) {
          setMarketplaceStatus('Stake amount must meet or exceed the minimum juror stake defined by the contract.')
          return
        }
  ...
  const handleFetchOpenTasks = useCallback(async () => {
    if (!typedAddress) {
      setOpenTasks([])
      setOpenTasksError('Enter a valid marketplace contract address to continue.')
      return
    }
    ...
    <div className="container">
      ...
      {!isAuthenticated && (
        <section className="card intro-card">
          ...
          <p className="muted">Use MetaMask or another injected wallet to access the marketplace tools.</p>
        </section>
      )}
      ...
      {isAuthenticated && (
        <section className="card marketplace-card">
          ...
          <p className="muted">
            Paste the deployed <code>TaskMarketplace</code> address and use these helpers to post
            tasks, submit solutions, verify work, and inspect stored task data.
          </p>

          <div className="field-group">
            <label htmlFor="marketplaceAddress">Marketplace contract address</label>
            <input
            ...
            <small className="hint">
              Configure <code>VITE_MARKETPLACE_ADDRESS</code> in your environment to prefill this
              field.
            </small>
          </div>
                ...
                {openTasks.map((task) => (
                  <li key={task.id.toString()} className="open-task-item">
                    <div className="open-task-heading">
                      ...
                      <span className="open-task-bounty">{formatEther(task.bountyWei)} ETH bounty</span>
                    </div>
        ...
        <form className="form-grid" onSubmit={handlePostTask}>
          <div className="field-group full-width">
            ...
            />
            <small className="hint">
              We'll generate an IPFS CID for this text in your browser if you leave the description
          ...
          <div className="field-group">
            ...
            />
            <small className="hint">Determines how long solvers have before the on-chain deadline.</small>
          </div>
          <button type="submit" disabled={!typedAddress || !isConnected || Boolean(pendingAction)}>
            Post task
          </button>
        </form>
        ...
        <form className="form-grid" onSubmit={handleJurorVote}>
          ...
          <div className="field-group">
            ...
            <input
              ...
              placeholder={typedAddress ? 'Loading minimum stake...' : 'Connect to view requirements'}
              readOnly
            />
          </div>
          <div className="field-group">
            <label htmlFor="jurorStakeAmount">Stake amount ({governanceTokenSymbolLabel})</label>
            <input
              id="jurorStakeAmount"
              type="text"
              inputMode="decimal"
          ...
          <div className="task-details">
            ...
            <dl>
              ...
              <div>
                ...
                <dd>
                  <code>
                    {fetchedTask.solver === zeroAddress ? '—' : fetchedTask.solver}
                  </code>
                </dd>
              </div>
              <div>
                ...
                <dd>
                  <code>
                    {fetchedTask.verifier === zeroAddress ? '—' : fetchedTask.verifier}
                  </code>
                </dd>
              </div>
              <div>
